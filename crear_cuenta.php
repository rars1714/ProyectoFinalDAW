            ...
            <div class="align-self-center collapse navbar-collapse flex-fill  d-lg-flex justify-content-lg-between" id="templatemo_main_nav">
                <div class="flex-fill">
                    <ul class="nav navbar-nav d-flex justify-content-between mx-lg-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="shop2.php">Nuevos Lanzamientos</a>
                        </li>
                    </ul>
                </div>
            ...
            <form method="post" role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                ...
                <div class="row">
                    <div class="col text-end mt-2">
                        <button type="submit" class="btn btn-success btn-lg px-3" name="registrar">Registrar</button>
                    </div>
                </div>
            </form>
        </div>
...
$con = new mysqli($server, $user, $contra, $db);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Escapar variables para evitar inyección de SQL
    $nombre = mysqli_real_escape_string($con, $_POST['nombre']);
    $apellido = mysqli_real_escape_string($con, $_POST['apellido']);
    $correo = mysqli_real_escape_string($con, $_POST['correo']);
    $pass = mysqli_real_escape_string($con, $_POST['pass']);
    $fnam = mysqli_real_escape_string($con, $_POST['fnam']);
    $direccion = mysqli_real_escape_string($con, $_POST['direccion']);

    $sql = "INSERT INTO usuarios (nombre, apellido, pass, correo, direccion, fnam)
            VALUES ('$nombre', '$apellido', '$pass', '$correo', '$direccion', '$fnam')";

    // Ejecutar la consulta
    if ($con->query($sql) === TRUE) {
        $last_id = $con->insert_id;  // Obtener el último ID insertado
        // Puedes redirigir al usuario a una página de éxito o realizar otras acciones aquí
    } else {
        echo "Error en el registro: " . $sql . "<br>" . $con->error;
    }
}
